...
const DersTakvimi = () => {
  ...
  const handleAddSchedule = async () => {
    ...
    if (isClassroomConflict) {
      alert("Derslik Çakışması tesbit edildi!");
      return;
    }
    ...
    if (isInstructorConflict) {
      alert("Öğretim Üyesi Çakışması Tespit Edildi!");
      return;
    }
  ...
  const handleUpdateSchedule = async () => {
    ...
    try {
      const response = await axios.put("http://localhost:8800/dersTakvimi/" + currentCourse.courseCalendarID, updatedCourseData);
      if (response.status === 200) {
